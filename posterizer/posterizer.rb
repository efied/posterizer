...
class Posterizer
  ...
  def self.parse(html)
    ...
    html.insert(html.index("</head>"), "<script type='text/javascript' src='http://ajax.googleapis.com/ajax/libs/jquery/1.6.4/jquery.min.js'> </script>") if html.index("</head>")
    html.insert(html.index("</head>"), "<script type='text/javascript' src='javascripts/posterizer.js'> </script>") if html.index("</head>")
    html
  end
  ...
  def self.parse_post_block(html, settings)
    ...
    settings["Post"][0].each do |key, value|
      ...
      if key["Comments"]
        html_dump = ""
        html_dump << '<div id="post_commentarea" style="display:none"><div class="posterousListComments"><div class="commentunit"><div class="comment_spacer">&nbsp;</div><div class="comment"><div class="comment_hide_button"><a href="#">&laquo;&nbsp;Hide</a></div><h4>Recent comments</h4></div></div><div id="pcomment_comments_71944530">'
        value.each do |comment|
          html_dump << %Q{<div class="commentunit" id="comment_9722302"><a name="pcomment_commentunit_9722302"></a><div class="comment_spacer">&nbsp;</div><div class="comment"><div class="comment_avatar"><a href="http://posterous.com/people/gthkGokKm"><img src="http://files.posterous.com/user_profile_pics/1467464/jess_field_thumb.jpg" class="profile_border" /></a></div><div class="commentname"><a href="#">#{comment["OwnerName"]}</a> said...</div>#{comment["Content"]}<ul class="mini_commands" id="mini_commands2_9722302"><li><a href="#">Remove comment</a></li></ul></div></div>}
        end
        ...
        html_dump << %Q{<p><a href="#" id="comment_link">Comments</a> [#{value.count}]</p>'}
